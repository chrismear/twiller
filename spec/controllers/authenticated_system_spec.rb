...
describe SessionsController do
  ...
  describe "logout_killing_session!" do
    ...
    it 'nils the current person'      do logout_killing_session!; current_person.should be_nil end
    it 'kills :person_id session' do
      session.stub!(:[]=)
      session.should_receive(:[]=).with(:person_id, nil).at_least(:once)
      logout_killing_session!
    end
    it 'forgets me' do    
      current_person.remember_me
      current_person.remember_token.should_not be_nil; current_person.remember_token_expires_at.should_not be_nil
      Person.find(1).remember_token.should_not be_nil; Person.find(1).remember_token_expires_at.should_not be_nil
      logout_killing_session!
      Person.find(1).remember_token.should     be_nil; Person.find(1).remember_token_expires_at.should     be_nil
    end
  end
  ...
  describe "logout_keeping_session!" do
    ...
    it 'nils the current person'      do logout_keeping_session!; current_person.should be_nil end
    it 'kills :person_id session' do
      session.stub!(:[]=)
      session.should_receive(:[]=).with(:person_id, nil).at_least(:once)
  ...
  describe "Logging in by cookie" do
    def set_remember_token token, time
      @person[:remember_token]            = token; 
      @person[:remember_token_expires_at] = time
      @person.save!
    end    
    before do 
